...
def extract_key_words(text, pagerank, threshold, word_similarity_weight):
    ...
    remove_plural_text = remove_plural(text)
    if pagerank:
        return get_pagerank_keywords(graph)
    else:
        try:
            keywords = get_degree_centrality_keywords(graph, text)
        except:
            # In case that keywords are not enough, we try to use another method.
            keywords = extract_key_words(text, pagerank=False, threshold=threshold, word_similarity_weight=word_similarity_weight - 0.1)
        return keywords
